...
export fn studio(direction: vec3f, blur: f32, backdrop: vec3f, angle: f32) -> vec3f {
  let behind = smoothstep(0.15, -0.35, direction.z);
  let height = direction.y * 0.5 + 0.5;
  let room = mix(mix(vec3f(0.012), vec3f(0.07), height * height), backdrop * 0.92, behind);
  let lit = turned(direction, angle);
